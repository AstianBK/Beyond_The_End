...
public class PortalShape {
    private static final BlockBehaviour.StatePredicate FRAME = (state, level, pos) -> state.is(Blocks.GLOWSTONE);
    ...
    public static Optional<PortalShape> findEmptyAetherPortalShape(LevelAccessor level, BlockPos bottomLeft, Direction.Axis axis) {
    ...
    public static Optional<PortalShape> findPortalShape(LevelAccessor level, BlockPos bottomLeft, Predicate<PortalShape> predicate, Direction.Axis axis) {
        ...
        if (optional.isPresent()) {
            ...
            return Optional.of(new PortalShape(level, bottomLeft, directionAxis)).filter(predicate);
        }
    }
    ...
    private int getDistanceUntilTop(BlockPos.MutableBlockPos mutablePos) {
        for (int i = 0; i < 21; ++i) {
            ...
            for (int j = 0; j < this.width; ++j) {
                ...
                if (blockState.is(BkCommonRegistry.PORTAL.get())) {
    ...
    private static boolean isEmpty(BlockState state) {
        return state.isAir() || state.is(Blocks.WATER) || state.is(BkCommonRegistry.PORTAL.get());
    }
    ...
    public void createPortalBlocks() {
        BlockState blockState = BkCommonRegistry.PORTAL.get().defaultBlockState().setValue(NetherPortalBlock.AXIS, this.axis);
