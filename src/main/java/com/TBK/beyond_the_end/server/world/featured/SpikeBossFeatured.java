package com.TBK.beyond_the_end.server.world.featured;
...
public class SpikeBossFeatured extends Feature<SpikesBossConfiguration> {
    private static final LoadingCache<Long, List<SpikeBossFeatured.EndSpike>> SPIKE_CACHE = CacheBuilder.newBuilder().expireAfterWrite(5L, TimeUnit.MINUTES).build(new SpikeBossFeatured.SpikeCacheLoader());

    public SpikeBossFeatured(Codec<SpikesBossConfiguration> p_66852_) {
        super(p_66852_);
    }

    public static List<SpikeBossFeatured.EndSpike> getSpikesForLevel(WorldGenLevel p_66859_) {
    ...
    public boolean place(FeaturePlaceContext<SpikesBossConfiguration> p_160372_) {
        SpikesBossConfiguration spikeconfiguration = p_160372_.config();
        WorldGenLevel worldgenlevel = p_160372_.level();
        RandomSource randomsource = p_160372_.random();
        BlockPos blockpos = p_160372_.origin();
        List<SpikeBossFeatured.EndSpike> list = spikeconfiguration.getSpikes();
        if (list.isEmpty()) {
            list = getSpikesForLevel(worldgenlevel);
        }

        for(SpikeBossFeatured.EndSpike spikefeature$endspike : list) {
    ...
    private void placeSpike(ServerLevelAccessor p_225247_, RandomSource p_225248_, SpikesBossConfiguration p_225249_, SpikeBossFeatured.EndSpike p_225250_) {
        ...
        for(BlockPos blockpos : BlockPos.betweenClosed(new BlockPos(p_225250_.getCenterX() - i, p_225247_.getMinBuildHeight(), p_225250_.getCenterZ() - i), new BlockPos(p_225250_.getCenterX() + i, p_225250_.getHeight() + 10, p_225250_.getCenterZ() + i))) {
            if (blockpos.distToLowCornerSqr((double)p_225250_.getCenterX(), (double)blockpos.getY(), (double)p_225250_.getCenterZ()) <= (double)(i * i + 1) && blockpos.getY() < p_225250_.getHeight()) {
                this.setBlock(p_225247_, blockpos, Blocks.AMETHYST_BLOCK.defaultBlockState());
    ...
    public static class EndSpike {
        public static final Codec<SpikeBossFeatured.EndSpike> CODEC = RecordCodecBuilder.create((p_66890_) -> {
            ...
            })).apply(p_66890_, SpikeBossFeatured.EndSpike::new);
        });
    ...
    static class SpikeCacheLoader extends CacheLoader<Long, List<SpikeBossFeatured.EndSpike>> {
        public List<SpikeBossFeatured.EndSpike> load(Long p_66910_) {
            IntArrayList intarraylist = Util.toShuffledList(IntStream.range(0, 10), RandomSource.create(p_66910_));
            List<SpikeBossFeatured.EndSpike> list = Lists.newArrayList();
            ...
            for(int i = 0; i < 10; ++i) {
                ...
                list.add(new SpikeBossFeatured.EndSpike(j, k, i1, j1, flag));
            }
